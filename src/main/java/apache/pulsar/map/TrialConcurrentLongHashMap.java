 ...
 * There is no guarantee that if we write value1 and value2, and are able to see value2, then we
 * will also see value1. In some cases, it is even possible to encounter two mappings with the same
 * key, leading the keys method to return a List containing two identical keys. <br>
 * It is crucial to understand that the results obtained from aggregate status methods such as keys
 * and values are typically reliable only when the map is not undergoing concurrent updates from
 * other threads. When concurrent updates are involved, the results of these methods reflect
 * transient states that may be suitable for monitoring or estimation purposes, but not for program
 * control.
 *
 * @param <V>
 */
@SuppressWarnings("unchecked")
public class TrialConcurrentLongHashMap<V> {
  ...
  public static class Builder<T> {
    ...
    public TrialConcurrentLongHashMap<T> build() {
      return new TrialConcurrentLongHashMap<>(
          expectedItems,
          concurrencyLevel,
          mapFillFactor,
          mapIdleFactor,
  ...
  @Deprecated
  public TrialConcurrentLongHashMap() {
    this(DefaultExpectedItems);
  }

  @Deprecated
  public TrialConcurrentLongHashMap(int expectedItems) {
    this(expectedItems, DefaultConcurrencyLevel);
  }

  @Deprecated
  public TrialConcurrentLongHashMap(int expectedItems, int concurrencyLevel) {
    this(
        expectedItems,
        concurrencyLevel,
  ...
  public V put(long key, V value) {
    ...
    int h = hash(key);
    return getSection(h).put(key, value, h, false, null);
  }

  public V putIfAbsent(long key, V value) {
    requireNonNull(value);
    int h = hash(key);
    return getSection(h).put(key, value, h, true, null);
  }

  public V computeIfAbsent(long key, LongFunction<V> provider) {
    requireNonNull(provider);
    int h = hash(key);
    return getSection(h).put(key, null, h, true, provider);
  }

  public V remove(long key) {
    int h = hash(key);
    return getSection(h).remove(key, null, h);
  }

  public boolean remove(long key, Object value) {
    requireNonNull(value);
    int h = hash(key);
    return getSection(h).remove(key, value, h) != null;
  }
   ...
   *
   * <p><b>Warning: Do Not Guarantee Thread-Safety.</b>
   *
  ...
  private static final class Section<V> extends StampedLock {
      ...
      try {
        while (true) {
          ...
          if (storedKey == key) {
            ...
            // Found an empty bucket. This means the key is not in the map. If we've already seen a
            // deleted
      ...
      try {
        ...
        if (autoShrink && size < resizeThresholdBelow) {
          try {
            ...
            int newCapacity =
                Math.max(alignToPowerOfTwo((int) (capacity / shrinkFactor)), initCapacity);
    ...
    public void forEach(EntryProcessor<V> processor) {
      ...
      // Go through all the buckets for this section. We try to renew the stamp only after a
      // validation
